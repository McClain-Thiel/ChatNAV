    ...
    <div className="animate-reveal" style={{ maxWidth: 860, margin: '0 auto', padding: '44px 40px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0, marginBottom: 32 }}>
        ...
        <div style={{
          ...
          {[
            { key: 'Source', value: result.benchmark_source },
            { key: 'Recall@20', value: result.recall_at_20.toFixed(2) },
            { key: 'PPV/epitope', value: result.ppv_per_epitope.toFixed(2) },
            { key: 'Epitopes selected', value: String(result.n_neoantigens) },
          ].map(item => (
      ...
      <div style={{
        ...
        {[
          { label: 'Neoantigens', value: result.n_neoantigens },
          { label: 'mRNA Length', value: `${result.mrna_length_nt.toLocaleString()} nt` },
          { label: 'Polyepitope', value: `${result.polyepitope_aa} aa` },
          { label: 'GC Content', value: `${result.gc_pct.toFixed(1)}%` },
          { label: 'CAI', value: result.cai.toFixed(3) },
          { label: 'MFE', value: `${result.mfe_kcal_mol.toFixed(1)}` },
        ].map((stat, i) => (
